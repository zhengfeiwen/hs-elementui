...
export const getI18nSettings = () => {
  return {
    dayNamesShort: weeks.map(week => t(`hs.datepicker.weeks.${week}`)),
    dayNames: weeks.map(week => t(`hs.datepicker.weeks.${week}`)),
    monthNamesShort: months.map(month => t(`hs.datepicker.months.${month}`)),
    monthNames: months.map((month, index) => t(`hs.datepicker.month${index + 1}`)),
    amPm: ['am', 'pm']
  }
}
