...
export default class HsTinymce extends Vue {
  ...
  @Prop({ default: '请输入文字' }) private placeholder!: string
  @Prop({ default: '0' }) private border!: string
  // 是否展示图片标题
  @Prop({ default: !1 }) private imageCaption!: boolean
  // 高级设置
  @Prop({ default: !1 }) private imageAdvtab!: boolean
  // 图片上传后端地址
  @Prop({ default: '' }) private imagesUploadUrl!: string
  // 图片上传后端地址
  @Prop({ default: '' }) private imagesUploadBasePath!: string
  ...
  get initOptions () {
    return {
      ...
      // images_upload_url: this.imagesUploadUrl ? this.imagesUploadUrl : '',
      image_advtab: this.imageAdvtab,
      plugins: plugins,
  ...
  private imagesUploadHandler (blobInfo: any, succFun: any, failFun: any) {
    console.log(this.$listeners['images-upload-handler'], 'imagesUploadHandler')
