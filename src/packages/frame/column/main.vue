<template>
  <div :class="'hs-frame-container-column' + (leftShort ? ' short' : '')">
    ...
    <div :class="getRightClass">
      <hs-frame v-if="!custom" :more="more" :default-name="defaultName" :menu-name="menuName" @on-query="onQuery" @on-reset="onReset">
...
import HsFrame from '../main.vue'
@Component({
  name: 'hs-frame-column',
  components: {
    HsFrame
  }
})

export default class HsFrameColumn extends Vue {
  ...
  get getLeftClass () {
    return {
      'hs-frame-container-column-left': !0,
      'hs-frame-container-column-left-mini': this.prop['left-size'] === 'mini',
  ...
  get getRightClass () {
    return {
      'hs-frame-container-column-right': !0,
      'hs-frame-container-column-right-mini': this.prop['left-size'] === 'mini',
      left: !this.leftShort,
...
$circle-r: 20px;
$transition-s: .5s;
.hs-frame-container-column{
  width: 100%;
  height: 100%;
  display: flex;
  flex-direction: row;
  background-color: #EAEAEA;
  .hs-frame-container-column-left{
    ...
    padding: 6px;
    overflow: inherit;
    &.hs-frame-container-column-left-mini{
  ...
  .hs-frame-container-column-right{
    height: 100%;
    transition: width $transition-s;
    width: calc(100% - #{$left-width});
    &.hs-frame-container-column-right-mini{
